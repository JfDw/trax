...
_RNG_UPDATERS = [
    lambda x: x,
    lambda rng: math_lib.random.split(rng, 2)[0],
]
  ...
  def testTrain(self, layer_id, rng_updater_id, batch_size, trax_has_weights,
    ...
    Args:
      ...
      batch_size: an integer or `None`, the value for the `batch_size` argument
        in `FromTrax.__init__`.
      trax_has_weights: bool, whether to make the trax layer contain weights at
        the time when `FromTrax.build` is called.
      explicit_build: bool, whether to explicitly call `FromTrax.build`.
      use_model: bool, whether to build a `tf.keras.Model` out of the `FromTrax`
        layer and use the model to do the training instead of the bare layer. If
        `True`, we will also test checkpointing and restoring using the model.
    """
    with math_lib.use_backend("tf"):
      ...
      weights, state = trax_layer.init(input_sig)
      ...
      keras_layer = trax2keras.FromTrax(
